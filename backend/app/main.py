...
Phase 4 adds the public product/category browsing routes.
"""
...
# it came from (a service raising a DomainError, a route raising
# HTTPException, Pydantic rejecting a bad request body, or slowapi
# rejecting a rate-limited request) - come back as
# {"success": false, "message": "..."} instead of each library's own
# differently-shaped default. Route handlers never need to build an
# error response themselves; they just raise, and one of these catches it.

@app.exception_handler(DomainError)
async def domain_error_handler(request: Request, exc: DomainError) -> JSONResponse:
    """Catches every business-rule violation raised by the service layer
    (DuplicateEmailError, ProductNotFoundError, and everything else) -
    each exception carries its own `http_status`, so this handler never
    needs to change when a new domain error is introduced."""
    return JSONResponse(
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException) -> JSONResponse:
    ...
    the auth dependency, the min_price > max_price 422, etc.) and
    reshapes FastAPI's default `{"detail": "..."}` into the project's
...
async def validation_exception_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
    ...
    fields, a password that fails the strength check, an invalid `sort`
    value, page_size out of range, etc). `message` is a single
    human-readable summary of the first error; the full structured list
    stays available under `errors` for clients that want to highlight
    specific fields.
